...
const CoinPage = () => {
    ...
    if(!cryptoId) {
        return (
            <div className='min-h-screen flex items-center justify-center bg-gray-900 text-white'>
                <p>Error: No Crypto currency ID provided!</p>
            </div>
        )
    }
    ...
    useEffect(() => {
        const fetchData = async () => {
            setError(null);
            try {
                const detailsResult = await fetch(`https://api.coingecko.com/api/v3/coins/${cryptoId}`,requestOptions);
                if(!detailsResult.ok) throw new Error(`Error fetching coin details: ${detailsResult.statusText}`);
                setCoinDetails(await detailsResult.json());
                const chartResult = await fetch(`https://api.coingecko.com/api/v3/coins/${cryptoId}/market_chart?vs_currency=${currentCurrency.name}&days=${period}&interval=daily`,
                                    requestOptions);
                if(!chartResult.ok) throw new Error(`Error fetching chart data: ${chartResult.statusText}`);
                setChartData(await chartResult.json());
            } catch (err) {
    ...
    if (!coinDetails || !chartData) {
        return (
            <div className='min-h-screen flex items-center justify-center bg-gray-900'>
                <div className=' animate-spin rounded-full h-12 w-12 border-t-2 border-8 b-2 border-emerald-500'/>
            </div>
        )
    }
  ...
  return (
    <>
        ...
        px-4 sm:px-[5%] md:px-[8%] py-6'>
            ...
            border border-emerald-500/20'>
                ...
                <div className='text-center md:text-left'>
                    <h1 className='text-2xl md:text-3xl font-bold bg-gradient-to-r from-emerald-400 to-cyan-400 bg-clip-text text-transparent'>
                        ...
                        ({coinDetails.symbol?.toUpperCase()})</span>
                    </h1>
            ...
            <div className='mb-6 bg-gray-800/30 backdrop-blur-md p-4 rounded-xl border border-emerald-500/20'>
                <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 gap-2'>
                    ...
                    <div className='relative group'>
                        <select className="bg-gray-800/60 border border-emerald-500/30 rounded-lg px-3
                        ...
                        blur opacity-30 group-hover:opacity-50 transition duration-300 -z-10'></div>
                    </div>
                </div>
            ...
            <div className='spacec-y-3 md:space-y-0 md:grid md:grid-cols-2 gap-4'>
