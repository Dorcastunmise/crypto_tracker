...
const Navbar = () => {
  ...
  const [filtereredCoins, setFiltereredCoins] = useState([]);
  ...
  const inputHandler = (event) => {
    ...
    if(value === "") {
      ...
      setFiltereredCoins(suggestions.slice(0,5));
    }
  }

  return (
    <div>
      <nav className="flex md: flex-nowrap items-center justify-between gap-4 px-[5%] md:px[8%] lg:[10%]
      py-5 bg-gray-900/80 backdrop-blur-md border-b border-gray-700/30 sticky top-0 z-50">
        ...
        {/* Search Bar */}
        <form onSubmit={searchHandler} className="order-3 w-full md:order-2 md:w-auto fles-1 max-w-2x1
        mx-0 md:mx-4 relative">
          <div className="relative group">
            ...
            <div className="relative flex items-center">
              <input type="text" placeholder="Crypto details search..." value={input} onChange={inputHandler} required
              ...
              backdrop-blur-sm"/>
              <button type="submit" className="z-10 relative right-2 top-1/2 -translate-y-1/2 px-4 py-1.5 bg-gradient-to-r from-emerald-600 to-cyan-600 text-white rounded-full
              hover:scale-105 transition-all">
                <Search className="w-4 h-4 pointer-events-none" />
              </button>
            </div>
          </div>
          {filtereredCoins.length > 0 && (
            <ul className=" absolute w-full bg-gray-800/95 border-gray-700 mt-2 rounded-lg
            shadow-xl z-10 backdrop-blur-md">
              {filtereredCoins.map((coin,idx) => (
